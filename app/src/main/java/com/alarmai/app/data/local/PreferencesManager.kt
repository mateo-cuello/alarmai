package com.alarmai.app.data.local

import android.content.Context
import android.content.SharedPreferences
import com.alarmai.app.BuildConfig
...
class PreferencesManager(context: Context) {
    ...
    companion object {
        ...
        private const val KEY_BEEP_MUTED = "beep_streams_muted"
    }

    /**
     * Tracks whether [com.alarmai.app.data.repository.VoiceManager] currently has system
     * streams muted. Persisted so a process killed mid-listen can undo the mute on next launch
     * rather than leaving the device silent indefinitely.
     */
    fun saveBeepMuted(muted: Boolean) {
        prefs.edit().putBoolean(KEY_BEEP_MUTED, muted).apply()
    }

    fun wasBeepMuted(): Boolean {
        return prefs.getBoolean(KEY_BEEP_MUTED, false)
    }
    ...
    fun getGeminiModel(): String {
        ...
        // Coerce unknown values (e.g. a model retired by a later app version) back to the default
        // rather than sending an id the fallback chain has no position for.
        return if (saved != null && GeminiModels.isKnown(saved)) saved else GeminiModels.DEFAULT
    }
