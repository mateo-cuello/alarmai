...
class VoiceManagerTest {
    ...
    @Test
    fun testMuteBeepNeverTouchesMusicStream() {
        // Regression: muteBeep() used to silence STREAM_MUSIC, which is the stream TTS renders on.
        // That swallowed the start of every spoken reply and, if the process died mid-listen, left
        // the user's media volume at zero permanently.
        srMockStatic.`when`<Boolean> { SpeechRecognizer.isRecognitionAvailable(any()) }.thenReturn(true)
        ...
        voiceManager.startListening(onResult = {}, onError = {}, onRmsChanged = {})
        voiceManager.stopListening()

        verify(audioManager, Mockito.never())
            .adjustStreamVolume(eq(AudioManager.STREAM_MUSIC), anyInt(), anyInt())
    }

    @Test
    fun testMuteStatePersistedForCrossProcessRecovery() {
        srMockStatic.`when`<Boolean> { SpeechRecognizer.isRecognitionAvailable(any()) }.thenReturn(true)
        ...
        voiceManager.startListening(onResult = {}, onError = {}, onRmsChanged = {})
        verify(prefs).saveBeepMuted(eq(true))

        voiceManager.stopListening()
        verify(prefs).saveBeepMuted(eq(false))
    }
